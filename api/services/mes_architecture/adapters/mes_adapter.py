...
# Absolute imports using project root package structure
...
from api.services.mes_architecture.resolvers.factory_resolver import FactoryResolver
from api.services.mes_architecture.resolvers.intent_resolver import IntentResolver
...
from api.services.mes_architecture.recovery.base import MesRecoveryStrategy, MesRecoveryResult
...
class MESAdapter:
    ...
    def __init__(self, db: AsyncSession, current_user_id: Optional[str] = None):
        ...
        from api.services.mes_architecture.production.material_shortage_pause import MaterialShortagePause
        from api.services.mes_architecture.quality.defect_threshold_alert import DefectThresholdAlert
        from api.services.mes_architecture.equipment.failure_recovery import EquipmentFailureRecovery
    ...
    async def get_work_orders(self, criteria: WorkOrderQueryCriteria) -> dict:
        ...
        if not self._validate_factory_access(criteria.factory_id):
            raise HTTPException(
                status_code=403,
                detail=f"User {self.current_user_id} does not have access to factory {criteria.factory_id}"
            )
        
        raw_orders = await self.repository.find_work_orders(criteria)
        formatted_items = [
            self.response_formatter.format_work_order(wo) for wo in raw_orders
        ]
        total_count = await self.repository.count_work_orders(criteria)
    ...
    async def update_work_order_status(self, request: WorkOrderUpdateRequest) -> dict:
        ...
        try:
            ...
            valid_values = [s.value for s in WorkOrderStatus]
            raise HTTPException(
                status_code=400,
                detail=f"Invalid status value: {request.new_status}. Valid values: {valid_values}"
            )
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail=f"Guard condition '{e.guard_name}' failed")
        
        updated_order = await self.repository.update_work_order_status(
            order.id, request.new_status, self.current_user_id
        )
    ...
    async def create_production_report(self, report_data: ProductionReportRequest) -> ProductionReportResponse:
        ...
        report = await self.repository.create_production_report(report_data.dict())
    ...
    def _validate_factory_access(self, factory_id: str) -> bool:
        ...
        return factory_id in allowed or factory_id == "all"
    ...
    def _build_report_context(self, report_data) -> Dict[str, Any]:
        return {
            "material_id": getattr(report_data, 'material_id', None) or report_data.work_order_id,
