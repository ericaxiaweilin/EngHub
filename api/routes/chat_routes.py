...
SYSTEM_PROMPT = (
    ...
    "快捷命令（如「最近有哪些不良品？」「查询在制工单」「查询库存水平」「今天生产情况怎么样？」"
    "「设备运行状态如何？」「最近的仿真审计记录」）与工作流标签"
    "（如「质量异常分诊」「生产日度复盘」「全面合规检查」「预警情报简报」）"
    "一律通过 tool calling 由你选择并调用工具，后端不会按关键词强制执行。"
    ...
    "工具结果须用简洁中文表格/清单呈现，禁止只原样倾倒 JSON。\n"
