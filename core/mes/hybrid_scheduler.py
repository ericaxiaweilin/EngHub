...
def demonstrate_hybrid_scheduling():
    ...
    logger.info("混合排产引擎演示 - 数据驱动的高级计划与排程")
    ...
    # 6. 输出排程结果摘要
    if result.success:
        logger.info("所有订单已成功排产")
    else:
        logger.warning("%d 个订单未能排产: %s", len(result.unscheduled_orders), result.unscheduled_orders)
    
    # 7. 导出甘特图数据
    gantt = scheduler.export_gantt_data()
    for station_id, tasks in gantt.items():
        logger.info("  %s: %d 个任务", station_id, len(tasks))
    ...
    logger.info(
        "插单后排程指标: 准时交付率=%.1f%%, 资源利用率=%.1f%%, 总任务数=%d",
        metrics.get('on_time_delivery_rate', 0),
        metrics.get('avg_resource_utilization', 0),
        metrics.get('total_tasks', 0),
    )
    logger.info("混合排产演示完成")
