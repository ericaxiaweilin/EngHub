...
class ProductionDataCollector:
    ...
    def collect_operation_data(
        ...
        efficiency = (standard_time / actual_time * 100) if actual_time > 0 else 0
        logger.info(
            "采集工序数据：%s | 工单=%s, 工序=%d, 工位=%s | 数量=%d, 良品=%d, 不良=%d | 标准=%.0fs, 实际=%.1fs, 效率=%.1f%%",
            data_id, work_order_id, operation_sequence, station_id,
            quantity, good_qty, defect_qty, standard_time, actual_time, efficiency,
        )
    ...
    def confirm_operation_data(self, data_id: str, confirmed_by: str) -> bool:
        ...
        if data_id not in self.operation_data:
            logger.warning("数据 %s 不存在", data_id)
            return False
        ...
        logger.info("确认工序数据：%s by %s", data_id, confirmed_by)
        return True
    ...
    def record_equipment_status(
        ...
        logger.info("设备状态变更：%s -> %s (原因: %s)", equipment_id, status, reason_code or 'N/A')
...
def demonstrate_data_collection():
    ...
    # 4. 查询工艺能力
    logger.info("查询工艺能力数据...")
    cap = collector.get_process_capability("TM-X100", 10, "STATION-SMT-01")
    if cap:
        logger.info(
            "工艺能力: 产品=%s 工序=%d | 平均工时=%.1fs | 良率=%.1f%% | 样本数=%d | 稳定=%s",
            cap.product_code, cap.operation_sequence,
            cap.avg_actual_time, cap.yield_rate, cap.sample_count, cap.is_stable,
        )
    ...
    logger.info(
        "工位 %s: 操作数=%d, 效率=%.1f%%, 良品=%d, 良率=%.1f%%",
        eff['station_id'], eff['total_operations'],
        eff['avg_efficiency'], eff['total_good_qty'], eff['yield_rate'],
    )
    
    # 6. 设备 OEE
    logger.info("设备 OEE 分析...")
    oee = collector.get_equipment_oee("EQ-SMT-001")
    logger.info(
        "设备 %s: 时间稼动率=%.1f%%, 性能稼动率=%.1f%%, OEE=%.1f%%",
        oee['equipment_id'], oee['availability'], oee['performance'], oee['oee'],
    )
    
    # 7. 导出 APS 数据
    logger.info("导出 APS 排产数据...")
    aps_data = collector.export_for_aps()
    logger.info(
        "导出完成: 工艺能力=%d条, 工位效率=%d个, 设备OEE=%d个, 工序记录=%d, 设备记录=%d",
        len(aps_data['process_capabilities']),
        len(aps_data['station_efficiency']),
        len(aps_data['equipment_oee']),
        aps_data['total_operation_records'],
        aps_data['total_equipment_records'],
    )
